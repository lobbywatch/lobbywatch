...
# ./run_local_db_script.sh lobbywatch dbdump_struct interactive ; mv `cat last_dbdump_file.txt` lobbywatch.sql ; diff -u -w -B <(git show HEAD:lobbywatch.sql | perl -p -e's/AUTO_INCREMENT=\d+//ig') <(cat lobbywatch.sql | perl -p -e's/AUTO_INCREMENT=\d+//ig') | less
