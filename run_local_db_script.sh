...
# ./run_local_db_script.sh lobbywatchtest dbdump_struct interactive ; mv `cat last_dbdump_file.txt` lobbywatch.sql ; diff -u <(git show HEAD:lobbywatch.sql | perl -p -e's/AUTO_INCREMENT=\d+//ig') <(cat lobbywatch.sql | perl -p -e's/AUTO_INCREMENT=\d+//ig') | kompare -
