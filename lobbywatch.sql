...
CREATE TABLE `mv_parlamentarier` (
  ...
  `verguetungstransparenz` decimal(23,2) DEFAULT NULL,
  `refreshed_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Materialized View aktualisiert am',
  `anzahl_interessenbindung_tief` tinyint(3) unsigned DEFAULT NULL,
  `anzahl_interessenbindung_mittel` tinyint(3) unsigned DEFAULT NULL,
  `anzahl_interessenbindung_hoch` tinyint(3) unsigned DEFAULT NULL,
...
VIEW `v_parlamentarier_transparenz_calculated` AS select `count_table`.`parlamentarier_id` AS `parlamentarier_id`,`count_table`.`anzahl_interessenbindungen` AS `anzahl_interessenbindungen`,`count_table`.`anzahl_abgelaufene_interessenbindungen` AS `anzahl_abgelaufene_interessenbindungen`,`count_table`.`anzahl_interessenbindungen_alle` AS `anzahl_interessenbindungen_alle`,`count_table`.`anzahl_erfasste_verguetungen` AS `anzahl_erfasste_verguetungen`,if((`count_table`.`anzahl_interessenbindungen` > 0),round((`count_table`.`anzahl_erfasste_verguetungen` / `count_table`.`anzahl_interessenbindungen`),2),1) AS `verguetungstransparenz` from (select `interessenbindung`.`parlamentarier_id` AS `parlamentarier_id`,count(if(((`interessenbindung`.`parlamentarier_id` is not null) and (isnull(`interessenbindung`.`bis`) or (`interessenbindung`.`bis` > now()))),1,NULL)) AS `anzahl_interessenbindungen`,count(if(((`interessenbindung`.`parlamentarier_id` is not null) and (`interessenbindung`.`bis` is not null) and (`interessenbindung`.`bis` < now())),1,NULL)) AS `anzahl_abgelaufene_interessenbindungen`,count(if((`interessenbindung`.`parlamentarier_id` is not null),1,NULL)) AS `anzahl_interessenbindungen_alle`,count(if(((`interessenbindung`.`parlamentarier_id` is not null) and (isnull(`interessenbindung`.`bis`) or (`interessenbindung`.`bis` > now())) and (`interessenbindung`.`verguetung` is not null)),1,NULL)) AS `anzahl_erfasste_verguetungen` from (`v_parlamentarier_simple` `parlamentarier` left join `v_interessenbindung_jahr_raw` `interessenbindung` on((`interessenbindung`.`parlamentarier_id` = `parlamentarier`.`id`))) group by `interessenbindung`.`parlamentarier_id`) `count_table` ;
SET character_set_client      = @saved_cs_client ;
SET character_set_results     = @saved_cs_results ;
SET collation_connection      = @saved_col_connection ;
...
-- Dump completed on 2019-09-15 11:57:26
