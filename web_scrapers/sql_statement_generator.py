...
def update_function_of_zutrittsberechtigung(zutrittsberechtigung_id, function, date, pdf_date):
    query = "UPDATE `zutrittsberechtigung` SET `funktion` = '{0}', `notizen` = CONCAT_WS('{1}', notizen), `updated_visa` = '{2}', `updated_date` = STR_TO_DATE('{3}', '%d.%m.%Y %T'), `updated_by_import` = STR_TO_DATE('{3}', '%d.%m.%Y %T') WHERE `id` = {4};\n".format(
        ...
        "{0}/import/{1}: Funktion geändert (PDF {2})\\n\\n".format(
...
def end_zutrittsberechtigung(zutrittsberechtigung_id, date, pdf_date):
    query = "UPDATE `zutrittsberechtigung` SET `bis` = STR_TO_DATE('{0}', '%d.%m.%Y'), `notizen` = CONCAT_WS('{1}', notizen), `updated_visa` = '{2}', `updated_date` = STR_TO_DATE('{3}', '%d.%m.%Y %T'), `updated_by_import` = STR_TO_DATE('{3}', '%d.%m.%Y %T') WHERE `id` = {4};\n".format(
        _date_as_sql_string(date),
        "{0}/import/{1}: Bis-Datum gesetzt (PDF {2})\\n\\n".format(
            _date_as_sql_string(date), user, _datetime_as_sql_string(pdf_date)),
        "import",
        _datetime_as_sql_string(date),
        zutrittsberechtigung_id
...
def update_sekretariat_organisation(organisation_id, sekretariat, batch_time, pdf_date):
    query = "UPDATE `organisation` SET `sekretariat` = '{0}', `notizen` = CONCAT_WS('{1}', notizen), `updated_visa` = '{2}', `updated_date` = STR_TO_DATE('{3}', '%d.%m.%Y %T'), `updated_by_import` = STR_TO_DATE('{3}', '%d.%m.%Y %T') WHERE `id` = {4};\n".format(
        ...
        "{0}/import/{1}: Sekretariat geändert (PDF {2})\\n\\n".format(
...
def update_name_de_organisation(organisation_id, name_de, batch_time, pdf_date):
    query = "UPDATE `organisation` SET `name_de` = {}, `notizen` = CONCAT_WS('{}', notizen), `updated_visa` = '{}', `updated_date` = STR_TO_DATE('{}', '%d.%m.%Y %T'), `updated_by_import` = STR_TO_DATE('{}', '%d.%m.%Y %T') WHERE `id` = {};\n".format(
        ...
        "{0}/import/{1}: Name DE geändert (PDF {2})\\n\\n".format(_date_as_sql_string(batch_time), user, _datetime_as_sql_string(pdf_date)),
        "import",
        _datetime_as_sql_string(batch_time),
        _datetime_as_sql_string(batch_time),
        organisation_id
...
def update_name_fr_organisation(organisation_id, name_fr, batch_time, pdf_date):
    query = "UPDATE `organisation` SET `name_fr` = {}, `notizen` = CONCAT_WS('{}', notizen), `updated_visa` = '{}', `updated_date` = STR_TO_DATE('{}', '%d.%m.%Y %T'), `updated_by_import` = STR_TO_DATE('{}', '%d.%m.%Y %T') WHERE `id` = {};\n".format(
        ...
        "{0}/import/{1}: Name FR geändert (PDF {2})\\n\\n".format(_date_as_sql_string(batch_time), user, _datetime_as_sql_string(pdf_date)),
        "import",
        _datetime_as_sql_string(batch_time),
        _datetime_as_sql_string(batch_time),
        organisation_id
...
def update_name_it_organisation(organisation_id, name_it, batch_time, pdf_date):
    query = "UPDATE `organisation` SET `name_it` = {}, `notizen` = CONCAT_WS('{}', notizen), `updated_visa` = '{}', `updated_date` = STR_TO_DATE('{}', '%d.%m.%Y %T'), `updated_by_import` = STR_TO_DATE('{}', '%d.%m.%Y %T') WHERE `id` = {};\n".format(
        ...
        "{0}/import/{1}: Name IT geändert (PDF {2})\\n\\n".format(_date_as_sql_string(batch_time), user, _datetime_as_sql_string(pdf_date)),
        "import",
        _datetime_as_sql_string(batch_time),
        _datetime_as_sql_string(batch_time),
        organisation_id
...
def update_adresse_organisation(organisation_id, adresse_str, adresse_zusatz, adresse_plz, adresse_ort, batch_time, pdf_date):
    query = "UPDATE `organisation` SET `adresse_strasse` = {0}, `adresse_zusatz` = {1}, `adresse_plz` = {2}, `ort` = {3}, `notizen` = CONCAT_WS('{4}', notizen), `updated_visa` = '{5}', `updated_date` = STR_TO_DATE('{6}', '%d.%m.%Y %T'), `updated_by_import` = STR_TO_DATE('{6}', '%d.%m.%Y %T') WHERE `id` = {7};\n".format(
        ...
        _quote_str_or_NULL(_escape_string(adresse_plz)),
        _quote_str_or_NULL(_escape_string(adresse_ort)),
        "{0}/import/{1}: Adresse geändert (PDF {2})\\n\\n".format(
            _date_as_sql_string(batch_time), user, _datetime_as_sql_string(pdf_date)),
        "import",
        _datetime_as_sql_string(batch_time),
        organisation_id
...
def update_homepage_organisation(organisation_id, homepage, batch_time, pdf_date):
    query = "UPDATE `organisation` SET `homepage` = {0}, `notizen` = CONCAT_WS('{1}', notizen), `updated_visa` = '{2}', `updated_date` = STR_TO_DATE('{3}', '%d.%m.%Y %T'), `updated_by_import` = STR_TO_DATE('{3}', '%d.%m.%Y %T') WHERE `id` = {4};\n".format(
        _quote_str_or_NULL(_escape_string(homepage)),
        "{0}/import/{1}: Homepage geändert (PDF {2})\\n\\n".format(
            _date_as_sql_string(batch_time), user, _datetime_as_sql_string(pdf_date)),
        "import",
        _datetime_as_sql_string(batch_time),
        organisation_id
...
def update_alias_organisation(organisation_id, alias, batch_time, pdf_date):
    query = "UPDATE `organisation` SET `alias_namen_de` = {0}, `notizen` = CONCAT_WS('{1}', notizen), `updated_visa` = '{2}', `updated_date` = STR_TO_DATE('{3}', '%d.%m.%Y %T'), `updated_by_import` = STR_TO_DATE('{3}', '%d.%m.%Y %T') WHERE `id` = {4};\n".format(
        _quote_str_or_NULL(_escape_string(alias)),
        "{0}/import/{1}: Alias geändert (PDF {2})\\n\\n".format(
            _date_as_sql_string(batch_time), user, _datetime_as_sql_string(pdf_date)),
        "import",
        _datetime_as_sql_string(batch_time),
        organisation_id
