...
FOR EACH ROW thisTrigger: BEGIN
  ...
  -- https://stackoverflow.com/questions/8590526/mysql-select-from-without-one-field
  -- https://stackoverflow.com/questions/9122/select-all-columns-except-one-in-mysql
  -- remove virtual cols from list
  -- VIRTUAL GENERATED or VIRTUAL STORED for generated columns in EXTRA
  -- -- SET @sql = CONCAT('SELECT ', (SELECT GROUP_CONCAT(COLUMN_NAME) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = @table AND TABLE_SCHEMA = @schema AND EXTRA NOT LIKE '%VIRTUAL%'), ', null, ''insert'', null, NOW(), null FROM ', @table, ';');
  -- SET @table = 'branche';
  -- SELECT DATABASE() INTO @schema;
  -- SET @sql = CONCAT('INSERT INTO `', @table, '_log` SELECT ', (SELECT GROUP_CONCAT(COLUMN_NAME) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = @table AND TABLE_SCHEMA = @schema AND EXTRA NOT LIKE '%VIRTUAL%'), ' FROM `', @table, '` WHERE id = ', NEW.id, ';');
  -- PREPARE stmt FROM @sql;
  -- EXECUTE stmt;
  -- CALL addLog('branche', NEW.id);
  INSERT INTO `branche_log`
