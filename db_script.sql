...
DROP FUNCTION IF EXISTS myschema.urlencode;

DELIMITER |

CREATE FUNCTION input_pentaho_bod.urlencode (s VARCHAR(4096)) RETURNS VARCHAR(4096)
DETERMINISTIC
CONTAINS SQL
BEGIN
...
CONCAT(
  CASE parlamentarier.geschlecht
    WHEN 'M' THEN CONCAT('Sehr%20geehrter%20Herr%20', parlamentarier.nachname, '%0D%0A')
    WHEN 'F' THEN CONCAT('Sehr%20geehrte%20Frau%20', parlamentarier.nachname, '%0D%0A')
    ELSE CONCAT('Sehr%20geehrte(r)%20Herr/Frau%20', parlamentarier.nachname, '%0D%0A')
  END,
  '[Einleitung]%0D%0A',
  'Ihre%20Interessenbindungen:%0D%0A',
  GROUP_CONCAT(DISTINCT
    CONCAT('*%20', organisation.anzeige_name, IF(organisation.rechtsform IS NULL OR TRIM(organisation.rechtsform) = '', '', CONCAT(',%20', organisation.rechtsform)), IF(organisation.ort IS NULL OR TRIM(organisation.ort) = '', '', CONCAT(',%20', organisation.ort)), ',%20', interessenbindung.art, ',%20', interessenbindung.beschreibung, '%0D%0A')
    ORDER BY organisation.anzeige_name
    SEPARATOR '%20'
  ),
  '%0D%0A',
  'Ihre Gäste:%0D%0A',
  GROUP_CONCAT(DISTINCT
    CONCAT('*%20', zugangsberechtigung.name, ',%20', zugangsberechtigung.funktion, '%0D%0A')
    ORDER BY organisation.anzeige_name
    SEPARATOR '%20'
  ),
  '%0D%0A',
  'Mit freundlichen Grüssen,%0D%0A'
) email_text_for_url
