...
class UserIdentityCookieStorage
{
    ...
    public function SaveUserIdentity(UserIdentity $identity)
    {
        $expire = $this->CalculateCookieExpirationTime($identity->persistent);
         // afterburned  // afterburned setcookie(self::userNameCookie, $identity->userName, $expire);
        $this->SetPasswordCookieEncrypted($identity->password, $expire);
         // afterburned  // afterburned setcookie(self::persistentCookie, strval((int)$identity->persistent), $expire);
    }
    ...
    {
         // afterburned  // afterburned setcookie(self::passwordCookie, $this->identityCheckStrategy->GetEncryptedPassword($plainPassword), $expire);
    }
    ...
    {
         // afterburned  // afterburned setcookie($cookie, '', time() - 3600);
    }
}
