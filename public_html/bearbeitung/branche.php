 ...
 */
    ...
    class branchePage extends Page
    {
        ...
        protected function getFiltersColumns()
        {
            return array(
                ...
                new FilterColumn($this->dataset, 'name', 'name', $this->RenderText('Name')),
                new FilterColumn($this->dataset, 'name_fr', 'name_fr', $this->RenderText('Name Fr')),
                new FilterColumn($this->dataset, 'kommission_id', 'kommission_id_anzeige_name_mixed', $this->RenderText('Kommission NR')),
                new FilterColumn($this->dataset, 'kommission2_id', 'kommission2_id_anzeige_name_mixed', $this->RenderText('Kommission SR')),
        ...
        protected function setupColumnFilter(ColumnFilter $columnFilter)
        {
            ...
            $columnFilter
                ...
                ->enableSearchFor('kommission_id', true)
                ->enableSearchFor('kommission2_id', true)
                ->enableSearchFor('technischer_name', true)
        ...
        protected function setupFilterBuilder(FilterBuilder $filterBuilder, FixedKeysArray $columns)
        {
            ...
            $main_editor = new AutocompleteComboBox('kommission2_id_edit', $this->CreateLinkBuilder());
            $main_editor->setAllowClear(true);
            $main_editor->setMinimumInputLength(0);
            $main_editor->SetAllowNullValue(false);
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            if (GetCurrentUserGrantForDataSource('branche.interessengruppe')->HasViewGrant() && $withDetails)
            {
            //
            ...
            if (GetCurrentUserGrantForDataSource('branche.organisation')->HasViewGrant() && $withDetails)
            {
            //
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $editColumn = new UploadFileToFolderColumn('Symbol', 'symbol_abs', $editor, $this->dataset, false, false, '' . $GLOBALS["public_files_dir_abs"] /*afterburner*/  . '/branche_symbole/%id%', '%original_file_name%', $this->OnGetCustomUploadFilename, false);
            ...
            $editor->SetShowImage(true);
            $editColumn = new UploadFileToFolderColumn('Symbol', 'symbol_rel', $editor, $this->dataset, false, false, '' . $GLOBALS["public_files_dir_abs"] /*afterburner*/  . '/branche_symbole/%id%', '%original_file_name%', $this->OnGetCustomUploadFilename, false);
            $editColumn->SetReplaceUploadedFileIfExist(true);
            $editColumn->SetReadOnly(true);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
        ...
        protected function doRegisterHandlers() {
            $detailPage = new branche_interessengruppePage('branche_interessengruppe', $this, array('branche_id'), array('id'), $this->GetForeignKeyFields(), $this->CreateMasterDetailRecordGrid(), $this->dataset, GetCurrentUserGrantForDataSource('branche.interessengruppe'), 'UTF-8');
            ...
            $detailPage = new branche_organisationPage('branche_organisation', $this, array('branche_id'), array('id'), $this->GetForeignKeyFields(), $this->CreateMasterDetailRecordGrid(), $this->dataset, GetCurrentUserGrantForDataSource('branche.organisation'), 'UTF-8');
            ...
            $lookupDataset->setOrderByField('anzeige_name_mixed', GetOrderTypeAsSQL(otAscending));
            $lookupDataset->AddCustomCondition(EnvVariablesUtils::EvaluateVariableTemplate($this->GetColumnVariableContainer(), ''));
            $handler = new DynamicSearchHandler($lookupDataset, $this, 'filter_builder_kommission2_id_anzeige_name_mixed_search', 'id', 'anzeige_name_mixed', null);
            GetApplication()->RegisterHTTPHandler($handler);
    ...
    try
    {
        $Page = new branchePage("branche", "branche.php", GetCurrentUserGrantForDataSource("branche"), 'UTF-8');
